...
import { getProjectFiles } from "../project-files.server";
...
export async function generateOpenApiDocument(user?: SessionData | null) {
  ...
  const projectFileList = defaultProject?.id
    ? await getProjectFiles(defaultProject.id)
    : null;

  const projectFileExample = projectFileList?.[0] ?? null;

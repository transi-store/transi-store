...
File configuration (format, path template, target locales) is managed in the Transi-Store UI. The CLI fetches this information from the server at runtime via `GET /api/orgs/:org/projects/:slug`, so adding a locale or a new file requires no change in your consumer repositories.

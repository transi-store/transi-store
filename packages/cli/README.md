...
- `--branch`: The branch to download the translations from. Defaults to "main". Use `@all` to download all translations across every branch (no branch filtering). If omitted and you are inside a git repository on a feature branch, the current branch is detected and used automatically.
